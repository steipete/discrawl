...
func TestAnalyticsCommand(t *testing.T) {
	...
	t.Run("analytics with no subcommand prints usage", func(t *testing.T) {
		...
		require.Contains(t, out.String(), "digest")
		require.Contains(t, out.String(), "quiet")
		require.Contains(t, out.String(), "trends")
	})

	t.Run("analytics digest delegates to digest", func(t *testing.T) {
		var outAnalytics bytes.Buffer
		require.NoError(t, Run(ctx, []string{"--config", cfgPath, "--json", "analytics", "digest", "--since", "7d"}, &outAnalytics, &bytes.Buffer{}))
		var analyticsDigest map[string]any
		require.NoError(t, json.Unmarshal(outAnalytics.Bytes(), &analyticsDigest))

		var outDigest bytes.Buffer
		require.NoError(t, Run(ctx, []string{"--config", cfgPath, "--json", "digest", "--since", "7d"}, &outDigest, &bytes.Buffer{}))
		var digest map[string]any
		require.NoError(t, json.Unmarshal(outDigest.Bytes(), &digest))
	...
	t.Run("analytics trends json schema", func(t *testing.T) {
		...
		var payload map[string]any
		require.NoError(t, json.Unmarshal(out.Bytes(), &payload))
		require.Equal(t, float64(4), payload["weeks"])
	...
	t.Run("subcommands validate their own flags", func(t *testing.T) {
		cases := [][]string{
			{"--config", cfgPath, "analytics", "digest", "--bogus"},
